  ...
  return (
    <nav className='flex flex-wrap justify-between md:items-center text-white px-10 pt-6 md:px-20'>  
      <span className='text-x1 font-bold tracking-wide'>Portfolio</span>
        ...
        <ul className= {`${
          menu?"block" : "hidden"} 
          mx-24 p-y2 mt-4 font-semibold md:mt-5 bg-black px-2 rounded-xl bg-opacity-30 
      ...
      {showMenu?(
        <RiMenu2Line size={30} className='md:hidden absolute right-10 top-6 transition-all duaration-300'
           onClick={() => {
            openMenu(!menu);
        ...
        <RiCloseLine size={30} className='md:hidden absolute right-10 top-6 transition-all duaration-300'
        />
      )}
    </nav>
  )
